...
class TestVisualizations:
    ...
    def test_plot_string_interactions(self):
        ...
        intpartners = genes_of_interest_from_string(go_gene_list, 20, taxonomy_id)
        assert isinstance(intpartners, pd.DataFrame)
        assert not intpartners.empty

        df = plot_string_interactions(go_gene_list, go_gene_list, intpartners, False)
    ...
    def test_plot_idr_attributes(self):
        ...
        [query_genes_df, screen_to_phenotype_dict] = get_phenotypes_for_genelist(
            self.session, go_gene_list, organism
        )
        query_genes_list = list(query_genes_df["Value"])
        [similar_genes, overlap_genes] = get_similar_genes(
            self.conn, query_genes_list, screen_to_phenotype_dict
        )
        ...
        [
            ...
            similar_genes, overlap_genes, "int_test", "Phenotypes", 0, 5, False
        )
        ...
        [
            ...
            overlap_genes, similar_genes, "int_test", "Screens", 0, 0, False
        )
