...
class TestAttributes:
    ...
    @pytest.mark.parametrize(
        ...
        [
            pytest.param(
                "Homo sapiens", False, marks=pytest.mark.xfail(reason="may timeout")
            ),
    ...
    def test_get_organism_screenids(self, organism, empty):
        organism_screen_idlist = get_organism_screenids(self.session, organism)
        if empty:
    ...
    def test_get_phenotypes_for_genelist(self):
        ...
        [query_genes_df, screen_to_phenotype_dict] = get_phenotypes_for_genelist(
            self.session, gene_list, organism
        )
        ...
        [similar_genes, overlap_genes] = get_similar_genes(
            self.conn, gene_list, screen_to_phenotype_dict
        )
